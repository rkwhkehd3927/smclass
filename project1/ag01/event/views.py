...
def calendar(request):
  ...
  if request.method == "GET":
    ...
    attendance = Attendance.objects.filter(aId=aId)

    # Member 객체 가져오기
    member = Member.objects.filter(id=id)

    print("attendance: ",attendance,"member: ",member,"id: ",id)

    ## 매달 1일에 count 리셋
    first_day_of_month = today.replace(day=1)

    if today == first_day_of_month:
      attendance[0].count = 0 # 카운트 리셋
      attendance[0].aTicket = 0
      attendance[0].save()
      print("카운트 리셋 count",attendance[0].count)


    # aDate과 today 비교
    if attendance:
      if attendance[0].aDate != today:
        attendance[0].aDate = today
        attendance[0].save()
        attendance.update(count=F('count')+1) # 출석 횟수 추가
        attendance.update(count=F('aTicket')+1) # 응모권 개수 추가
        member.update(point=F('point')+10) # 앙포인트 10p 적립
        context = {"result":"success","count":attendance[0].count,"aTicket":attendance[0].aTicket,"point":member[0].point}
      else:
        context = {"result":"already_checked"}
    else:
      Attendance.objects.create(aId=aId,aDate=today,count=1,aTicket=1,point=10)
