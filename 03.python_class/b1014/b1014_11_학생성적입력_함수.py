...
s_title = ['번호','이름','국어','영어','수학','합계','평균','등수'] # 전역변수 # 점수묻기를 반복문으로 처리하기
stuNo = len(students)
...
def stu_input(stuNo,students):
  while True:
    ...
    # kor = int(input(f"{name} 학생의 국어점수를 입력하세요."))
    # eng = int(input(f"{name} 학생의 영어점수를 입력하세요."))
    # math = int(input(f"{name} 학생의 수학점수를 입력하세요."))
