...
name varchar2(20),
avg1 number,
avg2 number(5,2) -- 123.23 (총 5자리 중, 소수점은 2자리)
);
...
select * from empleyees;
select,employee_id,emp_name,salary from employees;
select EMP_NAME from employees;

-- (명령어)호출시에는 대소문자 구분하지 않음
SELECT EMP_NAME FROM EMPLOYEES;
select emp_name from employees;
...
select * from employees;
-- 사원월급이 달러로 표기되어있음 -> 원화로 환산하시오.
select emp_name,salary,salary*1384.66 from employees;
select emp_name,salary,to_char(salary*1384.66,'999,999,999') from employees; -- 자릿수
